...
import { add_gift, remove_gift } from 'Xmas/Store';
...
class Giftee extends React.Component {
    ...
    render() {
        ...
        return <div>
            ...
            <div> { sum > fair_share ? 'over budget' : 'under budget' }</div>
            ...
            this.props.gifts.map( g => 
                <li> { g.label } - ${ g.price }
                    <input type="button" value="remove"
                        onClick={ e => this.props.remove_gift( this.props.params.giftee, g.label ) } />
                </li> 
            )
        }   
    </ul>
